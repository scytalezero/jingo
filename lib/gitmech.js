...
var gitMech = {
  ...
  ls: function (pattern, callback) {
    ...
    if (process.platform === "win32") {
      //Use a filter that works with windows cmd shell
      params = [ "ls-tree", "--name-only", "-r", "HEAD", docSubdir, "| findstr /r \\.md" ]
    } else {
      params = [ "ls-tree", "--name-only", "-r", "HEAD", "--", docSubdir + pattern ]
    }
    gitExec(params, function (err, data) {
      if (err) {
        data = ''
      }
